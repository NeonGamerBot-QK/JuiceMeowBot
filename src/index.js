...
const db = new Database({ compression: true, store: new SQLiteStore('./data.db') })
...
async function getStuffFromEmail(email) {
    if(!email) return `Hmm no email found... maybe ask them for there email if u need it??`
    const data = await fetch("https://juice.hackclub.com/api/get-user-omg-moments?email="+encodeURIComponent(email))
    if(data.status !== 200) return `Hmm no email found... maybe ask them for there email if u need it??`
    const json = await data.json()
    if(!Array.isArray(json)) return `Hmm no data from email found... maybe ask them for there email if u need it??`
    return json.reverse().map((e,i)=>`- *${i+1}* ${e.status == "Pending" ? ":clock1:" : ":white_check_mark:"} \`${e.id}\`, description: \`\`\`${e.description}\`\`\` ${e.video?`<${e.video}|Video>`:""} -- Created at: ${dayjs(e.created_at).format(`YYYY-MM-DD`)} (${dayjs().to(dayjs(e.created_at))}) `).join('\n')
}
// juice help thing
...
if(await db.get(`help_request_${message.thread_ts}`)) {
    ...
    const helper_side_message_ts = (await db.get(`help_request_${message.thread_ts}`)).helper_side_message_ts
...
const helper_side_message = await   app.client.chat.postMessage({
            ...
            for(const e of (await db.get(`old_help_${message.user}`) || [])) {
                if(!e.message_link) e.message_link = await app.client.chat.getPermalink({
                    channel: `C08AXLAASJK`,
            ...
            await db.set(`old_help_${message.user}`, (await db.get(`old_help_${message.user}`) || []).map(e=> {
               if(oldReqs.find(e=> e.ts === e.message_ts)) e.message_link = oldReqs.find(e=> e.ts === e.message_ts).link
                return e;
            }))
            // console.log(oldReqs)
            console.log(userInfo.user)
            const stuffFromEmail = await getStuffFromEmail(userInfo.user.profile.email)
...
app.event('message', async ({ message, say }) => {
    ...
    if(message.text.startsWith('?m')) {
        ...
        const macro = (await db.get(`macros_${message.user}`) || []).find(e=> e.name === name)
        if(!macro) return app.client.chat.postEphemeral({
            channel: message.channel,
...
app.event('message', async ({ message, say }) => {
    ...
    if(message.channel_type !== "im") return app.client.chat.postEphemeral({
        channel: message.channel,
    ...
    await db.set(`macros_${message.user}`, [...(await db.get(`macros_${message.user}`) || []), {
        name, 
